...
int main()
{
	...
	// How often (in frame) should we update teh HUD;
	...
	Sound relaodFailed;
	relaodFailed.setBuffer(reloadFailedBuffer);
		...
		*/
		...
		while (window.pollEvent(event))
		{
			if (event.type == Event::KeyPressed)
			{
					...
					state == State::GAME_OVER)
				{
					...
					// Prepare the gun and the ammo for the next game;
					currentBullet = 0;
					bulletsSpare = 24;
					bulletsInClip = 6;
					clipSize = 6;
		...
		if (state == State::PLAYING)
		{
			...
			if (framesSinceLastHUDUpdate > fpsMeasurementFrameInterval)
			{
				...
				std::stringstream ssZombesAlive;
				...
				// Update the High Score Text;
				ssZombesAlive << "Zombies:" << numZombiesAlive;
				zombieRemainingText.setString(ssZombesAlive.str());
